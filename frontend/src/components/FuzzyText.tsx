    ...
    const init = async () => {
      ...
      const run = timestamp => {
        ...
        if (direction === 'horizontal') {
          for (let j = 0; j < tightHeight; j += 2) { // Optimized loop step
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(offscreen, 0, j, offscreenWidth, 2, dx, j, offscreenWidth, 2);
          }
        } else if (direction === 'vertical') {
          for (let i = 0; i < offscreenWidth; i += 2) {
            const dy = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(offscreen, i, 0, 2, tightHeight, i, dy, 2, tightHeight);
          }
        } else {
          for (let j = 0; j < tightHeight; j += 2) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(offscreen, 0, j, offscreenWidth, 2, dx, j, offscreenWidth, 2);
          }
          ...
          for (let i = 0; i < offscreenWidth + fuzzRange; i += 2) {
            const dy = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange * 0.5);
            const colData = ctx.getImageData(i, 0, 2, tightHeight + fuzzRange);
            ctx.clearRect(i, -fuzzRange, 2, tightHeight + 2 * fuzzRange);
